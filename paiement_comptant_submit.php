...
function enterdata($id,$date_paiement,$payeur,$type,$numero_cheque,$organisme,$date_virement,$date_tresor,$info_tresor,$tpe,$montantcfp,$montanteuro,$mode,$echelonnage,$obs,$restearegler,$reglement,$table){
	...
	switch($mode) {
		case "num":
				...
				////next update facture
				$query = "UPDATE  `factures_".$table."` SET `reglement`='$reglement', `datereglement`='$date_paiement', `restearegler`='$restearegler',`comment`=CONCAT(COALESCE(`comment`,''),' ; ".date("d/m/y")." R&eacute;gl&eacute;e ($mode par $payeur)')  WHERE  `factures_".$table."`.`idfacture` = $id";
				$mysqli->query($query);
