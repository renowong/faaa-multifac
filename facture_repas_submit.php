...
$communefactureid = enterdata($fdata,$clientid);
...
function enterdata($fdata,$clientid){
$totalfcp = 0;
...
$today = date("Y-m-d");
	...
	$mysqli = new mysqli(DBSERVER, DBUSER, DBPWD, DB);
		...
		$query = "INSERT INTO `".DB."`.`factures_cantine` (`idfacture`, `idclient`,".
				 " `datefacture`, `communeid`, `montantfcp`, `montanteuro`, `restearegler`,`obs`,`repas`)".
				 " VALUES (NULL, '".$clientid."', '".$today."', '".$chrono."', '".$totalfcp."', '".$totaleuro."', '".$totalfcp."', '', '1')";
	//return $query;
	$mysqli->query($query);
...
// insert details now
	for($counter=0;$counter<count($fdata);$counter+=1){
		...
		$query = "INSERT INTO `".DB."`.`factures_cantine_details` (`iddetail`, `idfacture`,".
				...
				" VALUES (NULL, '".$lastid."', '".$detail[0]."', '".$detail[1]."', '0')";
		$mysqli->query($query);
	}
